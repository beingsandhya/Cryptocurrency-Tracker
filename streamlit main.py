...
if data is not None:
    ...
    st.dataframe(df.style.format({"Price (USD)": "${:.2f}", "Market Cap": "${:,.0f}", "24h Volume": "${:,.0f}", "24h % Change": "{:.2f}%"}))

    
    st.download_button("📥 Download Data", data=open("crypto_data.xlsx", "rb"), file_name="crypto_data.xlsx", mime="application/vnd.openxmlformats-officed
